...
maize_per_kg: number=0;
  ...
  if(this.consent==false){
    ...
    this.presentToast('The disability_type is required');
      ...
      this.db.addDetails(consent, farmers_name, do_you_have_disability,disability_type).then(data => {
        this.details = data;
...
//Display some inputs only when a given option has been selected.
hideShowMe1=false;
hideShow1() {
  if(this.hideShowMe1==false){
    this.hideShowMe1 = true;
  }
  else{
    this.hideShowMe1 = false;
  }
}
...
//list of land tenure
tenures = [
  "Privately owned with title_proof",
...
//submission data for centenary tool
async SubmitData(){
  /*
