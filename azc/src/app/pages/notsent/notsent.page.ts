...
acreage:acreage
        ...
        this.accsPrvds.postData(body, 'process21.php').subscribe((res:any)=> {
                ...
                loader.dismiss();
                this.disabledButton = false;
                this.AlertforOfflineSubmission_farmer_pt('Check your internet');
                console.log('Error ', err);
        });
  });
...
async AlertforOfflineSubmission_farmer_pt(a) {
  const alert = await this.alertCtrl.create({
    cssClass: 'my-custom-class',
    header: a,
    backdropDismiss: false,
  ...
  async presentAlertforDelete5(a:string, id5:number) {
    const alert = await this.alertCtrl.create({
      cssClass: 'my-custom-class',
      header: a,
      backdropDismiss: false,
