...
export interface Details6Interface {
...
export class DatabaseSentService {
  ...
  async getAllDetails() {
    let details6: Details6Interface[] = [];
    return this.sqlite.create({ name: 'farmer_sent3.db', location: 'default' }).then(
      (db6) => {
        this.dbInstance = db6;
        db6.executeSql('CREATE TABLE IF NOT EXISTS '
          + 'DETAILS(id6 INTEGER PRIMARY KEY AUTOINCREMENT,'
          +'consent TEXT,'
          +'farmers_name TEXT,'
  ...
  private getAllRecords(): Details6Interface[] {
    let details6: Details6Interface[] = [];
    this.dbInstance.executeSql('select * from DETAILS', []).then(
      (res) => {
        for(var x=0; x<res.rows.length; x++)
          details6.push(res.rows.item(x));
      }
    ).catch(e => {
      console.log(e);
    });
